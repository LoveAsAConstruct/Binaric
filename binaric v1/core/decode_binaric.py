...
def extract_bits_from_wav(wav_file, json_file, data_rate):
    ...
    for time in transition_times:
        ...
        mode_levels = Sxx[mode_indices, t_index] if mode_freqs else []
        ...
        threshold = np.max(power_levels) * 0.4
        header_threshold = np.max(header_levels) * 0.3  # Lower threshold for header
        mode_threshold = np.max(mode_levels) * 0.3

        bits = [1 if p > threshold else 0 for p in power_levels]
        header_bits = [1 if p > header_threshold else 0 for p in header_levels]
        mode_bits = [1 if p > mode_threshold else 0 for p in mode_levels]
        footer_bits = [1 if p > threshold else 0 for p in footer_levels]

        bit_array.append((time, header_bits, bits, footer_bits, mode_bits))

    return bit_array, t, Sxx, f, transition_times, data_indices, header_indices, footer_indices, mode_indices

def plot_spectrogram(wav_file, json_file, data_rate):
    """
    Plots the spectrogram with clock transition markers and bit value dots.
    """
    bit_data, t, Sxx, f, transition_times, data_indices, header_indices, footer_indices, mode_indicies = extract_bits_from_wav(wav_file, json_file, data_rate)
    ...
    for i, (time, header_bits, content_bits, footer_bits, mode_bits) in enumerate(bit_data):
        ...
        y_mode = [f[idx] for idx in mode_indicies]  # Footer bit frequencies
        ...
        # Mode bits (Red)
        ax.scatter([time] * len(y_mode), y_mode, c=['white' if b else 'black' for b in mode_bits], edgecolors='red', s=50, marker='o', label="Footer" if i == 0 else "")
